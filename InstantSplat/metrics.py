    ...
    for scene_dir in args.model_paths:
        try:
            ...
            for method in os.listdir(test_dir):
                ...
                # Per-image metrics
                out_f.write("Per-Image Metrics:\n")
                for idx in tqdm(range(len(renders)), desc="Metric evaluation progress"):
                    s = ssim(renders[idx], gts[idx])
                    p = psnr(renders[idx], gts[idx])
                    l = lpips(renders[idx], gts[idx], net_type='vgg')
                    out_f.write(f"Image Name: {image_names[idx]}, Image Index: {idx}, PSNR: {p.item():.2f}, SSIM: {s:.4f}, LPIPS: {l.item():.4f}\n")
                    ssims.append(s)
                    psnrs.append(p)
                    lpipss.append(l)
                ...
                out_f.write("\nAverage Metrics:\n")
                out_f.write(f"Average PSNR: {avg_psnr:.2f}\n")
                out_f.write(f"Average SSIM: {avg_ssim:.4f}\n")
                out_f.write(f"Average LPIPS: {avg_lpips:.4f}\n")
                out_f.close()

                print("  SSIM : {:>12.7f}".format(avg_ssim, ".5"))
                print("  PSNR : {:>12.7f}".format(avg_psnr, ".5"))
                print("  LPIPS: {:>12.7f}".format(avg_lpips, ".5"))

                full_dict[scene_dir][method].update({"SSIM": avg_ssim,
                                                     "PSNR": avg_psnr,
                                                     "LPIPS": avg_lpips})
