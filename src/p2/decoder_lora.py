...
class LoRALinear(nn.Module):
    ...
    The base linear is kept (can be frozen); only lora parameters are trainable.
    """
    def __init__(self, linear: nn.Linear, r: int = 4, alpha: int = 16, fan_in_fan_out: bool = False):
        super().__init__()
        self.fan_in_fan_out = fan_in_fan_out
        self.linear = linear  # keep original linear (weights can be frozen externally)
        ...
        if r > 0:
            # low-rank down and up projections
            self.lora_down = nn.Linear(in_features, r, bias=False)
            self.lora_up = nn.Linear(r, out_features, bias=False)
            # init: down small, up zeros
            nn.init.kaiming_uniform_(self.lora_down.weight, a=math.sqrt(5))
            nn.init.zeros_(self.lora_up.weight)

            # Ensure LoRA modules live on the same device/dtype as the wrapped linear
            dev = self.linear.weight.device
            dtype = self.linear.weight.dtype
            self.lora_down.to(device=dev, dtype=dtype)
            self.lora_up.to(device=dev, dtype=dtype)
        else:
            self.lora_down = None
            self.lora_up = None
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        base = self.linear(x)
        if self.r > 0 and self.lora_down is not None:
    ...
    def trainable_parameters(self):
        ...
        params = []
        if self.r > 0:
            params += list(self.lora_down.parameters())
            params += list(self.lora_up.parameters())
        return params
...
class Decoder(nn.Module):
    def __init__(self, config: Config, vision_dim: int = 768):
        super().__init__()
        self.config = config
    ...
    def add_lora(self, r: int = 4, alpha: int = 16, target_modules: Optional[list] = None):
        """
        Convert target linear layers in attention modules to LoRA-enabled layers.
    ...
    def forward(
        ...
        position_ids: Optional[torch.LongTensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        vision_embeds: Optional[torch.FloatTensor] = None,  # shape (batch, v_len, vision_dim)
    ) -> torch.FloatTensor:
        ...
        # If visual features are provided, project and prepend them as prefix tokens
        if vision_embeds is not None:
            # project vision tokens
            proj_v = self.visual_projection(vision_embeds)  # (B, v_len, hidden)
            # concatenate visual prefix before text tokens
            hidden_states = torch.cat([proj_v, inputs_embeds], dim=1)
            v_len = proj_v.size(1)
        else:
