...
model_path = "../pretrained/official_dino_model.pth"
...
#   print(val_dataset.data['label'].value_counts())
    ...
    num_epochs = 200
    save_every = 50
    save_backbone_dir = "./finetune_checkpoints/backbone"
    save_classifier_dir = "./finetune_checkpoints/classifier"
    os.makedirs(save_backbone_dir, exist_ok=True)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % save_every == 0:
            torch.save(backbone.state_dict(), f"{save_backbone_dir}/backbone_epoch{epoch}.pth")
            torch.save(classifier.state_dict(), f"{save_classifier_dir}/classifer_epoch{epoch}.pth")
